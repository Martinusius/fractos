...
import { createShader, renderer, render, Utils, setResolution, camera } from "./setup";
...
export class PathTracer {
  ...
  constructor(sdf: SDF = new SDF("cube()"), background: Background = new ColorBackground(new THREE.Color("white"))) {
  ...
  private initShader() {
    return createShader(core + pathTracer + this._sdf.glsl + this._background.glsl, {
  ...
  renderImage(width: number, height: number, time = 0) {
    ...
    return new Promise<TemporaryImage>((resolve, reject) => {
      ...
      const handleVisibilityChange = () => {
        if (document.visibilityState === "hidden") console.log("Render task paused");
      ...
      Queue.loop(
        () => {
          ...
          if (sample >= this.pixelDivisions * this.pixelDivisions) {
            ...
            console.log(
              `Render Size: ${
                Math.floor((this.pixelDivisions * this.pixelDivisions * width * height) / 100000) / 10
              } megapixels`
            );
            console.log(
              `Render Speed: ${
                Math.floor((this.pixelDivisions * this.pixelDivisions * width * height) / durationMs / 10) / 100
