...
float plane(float ro, float rd, float pn, float po, float size) {
    vec3 pn = normalize(vec3(1));
    ...
    return max(abs(dl), abs(p.y) / length(pn.xz)) < size ? 1.0 : 0.0;
}
...
MarchData march(vec3 direction) {
    ...
    for(int steps = 0; steps < maximumRaySteps; ++steps) {
        ...
        if(steps == 0) {
            if(dist < 0.0)
                return MarchData(vec3(0), cameraPos, true);
...
void main() {
    ...
    if(plane(cameraPos, rayDirection, vec3(1, 0, 0), vec3(0), 3.0)) {
        data.color += vec3(0.5, 0.5, 0);
    }
